document.addEventListener("DOMContentLoaded", function () {
  ...
  function init() {
    ...
    bottomContainer.style.height =
      (imageHeight + imageOffset) / 2 + imageOffset / 4;
  ...
  function onNextClick() {
    const imagesFromMainContainer =
      document.querySelectorAll("#container .image");
    for (let i = 0; i < imagesFromMainContainer.length; i++) {
      const image = imagesFromMainContainer[i];
      image.style.top = -1000;
    }

    const imagesFromBottomContainer = document.querySelectorAll(
      "#bottomContainer .image"
    );
    ...
    const imgFromBottomContainer = document.querySelectorAll(
      "#bottomContainer .image img"
    );
